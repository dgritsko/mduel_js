...
const runRight = {
    match: { location: locations.PLATFORM, right: true, down: false, position: [positions.DEFAULT, positions.CROUCHING] },
    act: Object.assign({ animation: animations.RUN }, basicState, moveRight)
}

const jumpLeft = {
    match: { location: locations.PLATFORM, left: true, up: true, down: false, position: positions.DEFAULT },
    act: Object.assign({ animation: animations.RUN_JUMP }, basicState, moveLeft, moveJump)
}

const jumpRight = {
    match: { location: locations.PLATFORM, right: true, up: true, down: false, position: positions.DEFAULT },
...
const crouchOrClimb = {
    match: { location: locations.PLATFORM, down: true, left: false, right: false },
    act: (player, level, state) => {
        const rope = nearbyRope(player, level);
...
const climbUpRope = {
    ...
    act: (player, level) => {
        player.applyState({ animation: animations.CLIMB, position: positions.DEFAULT, dy: -cfg.climbRate })
        stayOnRope(player);   
    }
}
...
const fallOffRopeLeft = {
    ...
    act: Object.assign({ animation: animations.STAND_FALL, location: locations.AIR }, basicState, moveLeft)
}

const fallOffRopeRight = {
    match: { location: locations.ROPE, right: true },
    act: Object.assign({ animation: animations.STAND_FALL, location: locations.AIR }, basicState, moveRight)
}
...
const handlePlayerMovement = (player, level) => {
    ...
    if (playerState.inputEnabled && playerState.anyInput) {
        for (let i = 0; i < behaviors.length; i++) {
            ...
            const behaviorScore = score(playerState, behavior.match);

            if (behaviorScore.actual === behaviorScore.target && bestScore < behaviorScore.actual) {
    ...
    if (!bestMatch && player.eventQueue.length === 0 && !playerState.anyInput) {
        switch (playerState.location) {
            case locations.PLATFORM:
                player.applyState({ animation: animations.STAND, xVelocity: 0, position: positions.DEFAULT })
                break;
            case locations.ROPE:
                player.applyState({ animation: animations.NONE, position: positions.DEFAULT })
                break;
        }
    }
}
